...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        self.fc_sigma2 = nn.Linear(n_features, z_dim)
        self.reverse_transform = nn.Linear(z_dim, n_features)
        # ========================
    ...
    def encode(self, x):
        ...
        # ====== YOUR CODE: ======
        x_encoded = self.features_encoder(x)
        x_encoded = x_encoded.reshape(x_encoded.shape[0], -1)

        mu, log_sigma2 = self.fc_mu(x_encoded), self.fc_sigma2(x_encoded)
        sigma2 = torch.exp(log_sigma2)

        p = torch.distributions.Normal(torch.zeros_like(mu), torch.ones_like(sigma2))
        u = p.rsample()
        z = mu + u * torch.sqrt(sigma2)
        # ========================
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    dim_x = x.shape[1] * x.shape[2] * x.shape[3]

    data_loss = res.norm(p=2, dim=1) ** 2
    data_loss = data_loss.mean() / (x_sigma2 * dim_x)

    z_sigma2 = torch.exp(z_log_sigma2)
    trace = z_sigma2.sum(dim=1).mean()

    mu_norm_square = z_mu.norm(p=2, dim=1) ** 2
    mu_norm_square = mu_norm_square.mean()

    log_det = z_sigma2.prod(dim=1)
    log_det = log_det.log().mean()

    kldiv_loss = trace + mu_norm_square - z_mu.shape[1] - log_det
