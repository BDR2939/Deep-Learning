...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    result = torch.zeros(text_length, num_chars, dtype=torch.int8)
    result[
        torch.arange(text_length), torch.tensor([char_to_idx[char] for char in text])
    ] = 1
    # ========================
    return result
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    text_length = len(text)
    embedded_text = torch.zeros(
        text_length, embedding_dim, device=device, dtype=torch.int8
    )
    embedded_text[
        torch.arange(text_length), torch.tensor([char_to_idx[char] for char in text])
    ] = 1
    num_samples = text_length // seq_len
    samples = embedded_text[: num_samples * seq_len].view(
        num_samples, seq_len, embedding_dim
    )
    labels_text = text[1 : num_samples * seq_len + 1]
    labels_indices = torch.tensor(
        [char_to_idx[char] for char in labels_text], dtype=torch.int8
    )
    labels = labels_indices.view(num_samples, seq_len)
    # ========================
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        input_seq = torch.unsqueeze(
            chars_to_onehot(start_sequence, char_to_idx), dim=0
        ).to(device, dtype=torch.float)
        hidden_state = None
        for _ in range(n_chars - len(start_sequence)):
            output, hidden_state = model(input_seq, hidden_state)
            probabilities = hot_softmax(output.squeeze(), dim=-1, temperature=T)
            sampled_index = torch.multinomial(probabilities, num_samples=1)[0].item()
            sampled_char = idx_to_char[sampled_index]
            out_text += sampled_char
            input_seq = torch.unsqueeze(
                chars_to_onehot(sampled_char, char_to_idx), dim=0
            ).to(device, dtype=torch.float)
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        indices = list(range(num_batches * self.batch_size))
        indices = torch.reshape(
            torch.tensor(indices), (self.batch_size, num_batches)
        ).t()
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(n_layers):
            ...
            self.add_module(f"layer_{i}", nn.ModuleDict(params))
        ...
        for t in range(seq_len):
            ...
            for i, params in enumerate(self.layer_params):
                w_xz, w_hz, w_xr, w_hr, w_xg, w_hg, drop = [
                    params[key]
                    for key in ("w_xz", "w_hz", "w_xr", "w_hr", "w_xg", "w_hg", "drop")
                ]
                h = layer_states[i]
