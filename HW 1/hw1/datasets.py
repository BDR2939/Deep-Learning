...
def random_labelled_image(
    ...
    label = torch.randint(0, num_classes, (1,), dtype=torch.int)
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            start, stop, step = index.indices(self.subset_len)
            indices = range(start + self.offset, stop + self.offset, step)
            return [self.source_dataset[i] for i in indices]
        elif index < 0 or index >= self.subset_len:
