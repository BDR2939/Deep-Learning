...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        reg[0, 0] = 0
        self.weights_ = np.linalg.solve(X.T @ X + reg, X.T @ y)
        return self
...
class BostonFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_transformed = poly_features.fit_transform(X_transformed)
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    """
    corr = df.corr()[target_feature].drop(target_feature, axis=0).abs()
    corr.sort_values(ascending=False, inplace=True)
    top_n_features = corr.index[:n]
    top_n_corr = corr.values[:n]
...
def cv_best_hyperparams(
    ...
    grid_search = sklearn.model_selection.GridSearchCV(model, param_grid, cv=kf, scoring='r2')
